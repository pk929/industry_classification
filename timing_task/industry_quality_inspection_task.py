...
def industry_quality_inspection_task():
    ...
    for industry_id in second_industry_id_list:
        # 查询行业标题
        result_industry_trace_titles = queryBySQL_sqlal(CcsTraceTitleBasicsMapper.S_TRACE_TITLE_BY_INDUSTRY_ID, {"industry_id":industry_id})
    ...
    ll = len(y_train)
    log.info(ll)
    random_list = random.sample(range(0, ll), int(ll/3)) # 随机取1/3数据测试
    ...
    stop_list = stopwordslist("E:/pycharm_python_workspace/industry_classification/text_similarity_modular/date/stopWord.txt")
